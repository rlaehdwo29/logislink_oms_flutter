...
class _OrderCargoInfoPageState extends State<OrderCargoInfoPage> {
  ...
  Future<void> initView() async {
    ...
    mData.value.inOutSctnName = await SP.getCodeName(Const.IN_OUT_SCTN, mData.value.inOutSctn??"");
    mData.value.truckTypeName = await SP.getCodeName(Const.TRUCK_TYPE_CD, mData.value.truckTypeCode??"");
    mData.value.carTypeName = await SP.getCodeName(Const.CAR_TYPE_CD, mData.value.carTypeCode??"");
    mData.value.carTonName = await SP.getCodeName(Const.CAR_TON_CD, mData.value.carTonCode??"");
    mData.value.itemName = await SP.getCodeName(Const.ITEM_CD, mData.value.itemCode??"");
    mData.value.sWayName = await SP.getCodeName(Const.WAY_TYPE_CD, mData.value.sWayCode??"");
    mData.value.eWayName = await SP.getCodeName(Const.WAY_TYPE_CD, mData.value.eWayCode??"");
                                        ...
                                        Expanded(
                                            ...
                                            child: Container(
                                                ...
                                                child: TextField(
                                                  ...
                                                  onChanged: (value){
                                                    if(value.length > 0) {
                                                      mData.value.goodsQty = int.parse(value.trim()).toString();
                                                      goodsQtyController.text = int.parse(value.trim()).toString();
                                                    }else{
                                                      goodsQtyController.text = "0";
                                                      mData.value.goodsQty = "0";
                                                    }
                                                  },
                                                  maxLength: 50,
